...
def get_ingrediente(id):
    try:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM ingredientes WHERE id_ingrediente = %s", (id,))
        f = cursor.fetchone()
        cursor.close()
        conn.close()
        if not f: return jsonify({"error": "No encontrado"}), 404
        return jsonify(f)
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def create_ingrediente():
    try:
        ...
        cursor.execute("""
            ...
            VALUES (%s, %s, %s, %s)
        """, (nombre, unidad, float(data.get("cantidad") or 0), float(data.get("costo_unitario") or 0)))
        conn.commit()
        cursor.close()
        conn.close()
...
def update_ingrediente(id):
    try:
        ...
        """, (data.get("nombre"), data.get("unidad"), float(data.get("cantidad") or 0), 
